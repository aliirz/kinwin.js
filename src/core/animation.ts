...
BaseKinWin.prototype.fadeIn = async function (duration = 300) {
  ...
  const promises = this.elements.map(
    (el) =>
      el.animate([Animation.TRANSITIONS.fade.out, Animation.TRANSITIONS.fade.in], {
        duration,
        fill: 'forwards',
      }).finished
  );
...
BaseKinWin.prototype.fadeOut = async function (duration = 300) {
  ...
  const promises = this.elements.map(
    (el) =>
      el.animate([Animation.TRANSITIONS.fade.in, Animation.TRANSITIONS.fade.out], {
        duration,
        fill: 'forwards',
      }).finished
  );
...
BaseKinWin.prototype.slideIn = async function (duration = 300) {
  ...
  const promises = this.elements.map(
    (el) =>
      el.animate([Animation.TRANSITIONS.slide.out, Animation.TRANSITIONS.slide.in], {
        duration,
        fill: 'forwards',
      }).finished
  );
